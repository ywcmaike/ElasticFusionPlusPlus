...
//                             , cv::Mat firstRGB, cv::Mat firstDepth, cv::Mat firstDescriptor, std::vector<cv::KeyPoint> firstKeyPoint,
//                             cv::Mat nextRGB, cv::Mat nextDepth, cv::Mat nextDescriptor, std::vector<cv::KeyPoint> nextKeyPoint
        ):
//        firstMyFrame(Resolution::getInstance().width(),
//                Resolution::getInstance().height(),
//                rgb, depth, descriptor, keyPoint),
//   nextMyFrame(Resolution::getInstance().width(),
//               Resolution::getInstance().height(),
//               rgb, depth, descriptor, keyPoint),
...
MyFrame & ElasticFusion::getFirstMyFrame()
{
    return firstMyFrame;
}
MyFrame & ElasticFusion::getNextMyFrame()
{
    return nextMyFrame;
}
/**
...
void ElasticFusion::setMyFrame(unsigned char *rgb, unsigned short *depth, MyFrame & myFrame)
{
    cv::Mat inputRGB(Resolution::getInstance().height(),
                     Resolution::getInstance().width(), CV_8UC3, rgb);
    cv::Mat inputDepth(Resolution::getInstance().height(),
                       Resolution::getInstance().width(), CV_16UC1, depth);
    myFrame.rgb = inputRGB;
    myFrame.depth = inputDepth;

    cv::Ptr<cv::FeatureDetector> _detector;
    cv::Ptr<cv::DescriptorExtractor> _descriptor;
    _detector = cv::FeatureDetector::create("ORB");
    _descriptor = cv::DescriptorExtractor::create("ORB");
    _detector->detect(myFrame.rgb, myFrame.keyPoint);
    _descriptor->compute(myFrame.rgb, myFrame.keyPoint, myFrame.descriptor);
    return;
}

void ElasticFusion::computeRotationAndTranslation(MyFrame& firstMyFrame, MyFrame& nextMyFrame,
                                                  Eigen::Matrix<float, 3, 3, Eigen::RowMajor> & initRot,
                                                  Eigen::Vector3f & initTrans, bool & initRt)
{
    cv::Mat rvec, tvec;
    int inliers;
    std::vector<cv::DMatch> matches;
    cv::BFMatcher matcher;
    matcher.match(firstMyFrame.descriptor, nextMyFrame.descriptor, matches);
    ...
    for (size_t i = 0; i < goodMatches.size(); i++)
    {
        ...
        cv::Point2f p = firstMyFrame.keyPoint[goodMatches[i].queryIdx].pt;
        //获取d要小心，x是向右的，y是向下的，所以y才是行，x是列
        ushort d = firstMyFrame.depth.ptr<ushort>(int(p.y))[int(p.x)];
        if (d == 0)
        {
            continue;
        }
        pts_img.push_back(cv::Point2f(nextMyFrame.keyPoint[goodMatches[i].trainIdx].pt));

        //将(u,v,d)转成(x, y,z);
        cv::Point3f pt(p.x, p.y, d);
        cv::Point3f pd;
        double scale = 1000.0;
        pd.z = double(pt.z) / scale;
        pd.x = (pt.x - Intrinsics::getInstance().cx()) * pd.z / Intrinsics::getInstance().fx();
        pd.y = (pt.y - Intrinsics::getInstance().cy()) * pd.z / Intrinsics::getInstance().fy();
    ...
    double camera_matrix_data[3][3] = {
            {Intrinsics::getInstance().fx(), 0, Intrinsics::getInstance().cx()},
    ...
    cv::Mat inlier;
    cv::solvePnPRansac(pts_obj, pts_img, cameraMatrix, cv::Mat(), rvec, tvec, false, 100, 1.0, 100, inlier);
    ...
    double norm = fabs(std::min(cv::norm(rvec), 2 * M_PI - cv::norm(rvec))) + fabs(cv::norm(tvec));
    double max_norm = 0.3;
    if (norm >= max_norm) {
        return;
    }
    initRt = true;
    cv::cv2eigen(rvec, initRot);
    cv::cv2eigen(tvec, initTrans);

    return;
}

void ElasticFusion::processFrame(Eigen::Matrix<float, 3, 3, Eigen::RowMajor> & initRot, Eigen::Vector3f & initTrans, bool initRt,
                                 ...
                                 const bool bootstrap)
{
    ...
    else
    {
        ...
        if(bootstrap || !inPose)
        {
            ...
            TICK("odom");
            frameToModel.getIncrementalTransformation(initRot, initTrans, initRt, trans,
                                                      rot,
                                                      rgbOnly,
                                                      icpWeight,
                                                      pyramid,
