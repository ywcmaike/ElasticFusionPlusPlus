   ...
   resizeStream(0)
{
    ...
    else
    {
        ...
        std::cout << "run here: OpenNI2....." << std::endl;
...
//
//          good = ((LiveLogReader *)logReader)->cam->ok();
//          std::cout << "run here: realsense....." << std::endl;
//        }
//#endif
...
//#ifdef WITH_REALSENSE2
        ...
        {
            delete logReader;
            logReader = new LiveLogReader(logFile, flipColors, LiveLogReader::CameraType::RealSense2);

            good = ((LiveLogReader *)logReader)->cam->ok();
            std::cout << "run here: realsense2....." << std::endl;
        }
//#endif
    ...
    while(!pangolin::ShouldQuit() && !((!logReader->hasMore()) && quiet) && !(eFusion->getTick() == end && quiet))
    {
        if(!gui->pause->Get() || pangolin::Pushed(*gui->step))
        {
            if((logReader->hasMore() || rewind) && eFusion->getTick() < end)
            {
                ...
                eFusion->setMyFrame(logReader->rgb, logReader->depth, firstMyFrame);
                ...
                MyFrame nextMyFrame;
                eFusion->setMyFrame(logReader->rgb, logReader->depth, nextMyFrame);

                bool initRt = false;
                Eigen::Matrix<float, 3, 3, Eigen::RowMajor> initRot;
                Eigen::Vector3f initTrans;
                eFusion->computeRotationAndTranslation(firstMyFrame, nextMyFrame, initRot, initTrans,
                                                       initRt);

                eFusion->processFrame(initRot, initTrans, initRt,
